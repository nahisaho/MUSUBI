...
module.exports = { helper };`
  ...
  describe('buildFocusedContext', () => {
    it('should build context for specific files', async () => {
      ...
      const context = await optimizer.buildFocusedContext(['src/index.js'], { maxTokens: 2000 });
    ...
    it('should truncate to fit token limit', async () => {
      ...
      const context = await optimizer.buildFocusedContext(['src/index.js', 'src/utils.js'], {
        maxTokens: 100,
      });
    ...
    it('should include AST symbols when enabled', async () => {
      ...
      const context = await optimizer.buildFocusedContext(['src/index.js'], { includeAST: true });
...
describe('optimizeContext', () => {
  it('should optimize context for path', async () => {
    ...
    await fs.promises.mkdir(testDir, { recursive: true });
    await fs.promises.writeFile(path.join(testDir, 'test.js'), 'function test() {}');
...
describe('Constants', () => {
  ...
  it('should have valid weight values', () => {
    for (const [_task, weights] of Object.entries(TASK_WEIGHTS)) {
      expect(
        weights.entryPoints || weights.errorLocation || weights.changedFiles || weights.targetFile
      ).toBeDefined();
